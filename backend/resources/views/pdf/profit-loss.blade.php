...
<head>
    <meta charset="utf-8">
    <title>Profit & Loss - {{ $data['from_date'] }} to {{ $data['to_date'] }}</title>
    <style>
...
<body>
        ...
        <table class="header-table">
            <tr>
                ...
                <td style="width:45%">
                    ...
                    <div class="report-meta">{{ \Carbon\Carbon::parse($data['from_date'])->format('d M Y') }} — {{ \Carbon\Carbon::parse($data['to_date'])->format('d M Y') }}</div>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table class="data">
        <tbody>
            @forelse($data['income_accounts'] as $a)
                <tr>
                    <td style="width:70%">{{ $a['name'] }} <span style="color:#999">({{ $a['code'] }})</span></td>
                    <td class="num" style="width:30%">{{ number_format($a['normal_balance'] === 'credit' ? $a['balance'] : -$a['balance'], 2) }}</td>
                </tr>
            @empty
            ...
            <tr class="subtotal-row">
                <td>Total Income</td>
                <td class="num">Rs. {{ number_format($data['total_income'], 2) }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <table class="data">
        <tbody>
            @forelse($data['expense_accounts'] as $a)
                <tr>
                    <td style="width:70%">{{ $a['name'] }} <span style="color:#999">({{ $a['code'] }})</span></td>
                    <td class="num" style="width:30%">{{ number_format($a['normal_balance'] === 'debit' ? $a['balance'] : -$a['balance'], 2) }}</td>
                </tr>
            @empty
            ...
            <tr class="subtotal-row">
                ...
                <td class="num">Rs. {{ number_format($data['total_expenses'], 2) }}</td>
            </tr>
        </tbody>
    </table>

    <table class="summary-table">
        <tr class="income"><td>Total Income</td><td class="num">Rs. {{ number_format($data['total_income'], 2) }}</td></tr>
        <tr class="expense"><td>Total Expenses</td><td class="num">(Rs. {{ number_format($data['total_expenses'], 2) }})</td></tr>
        <tr class="net-row">
            <td>{{ $data['net_profit'] >= 0 ? 'NET PROFIT' : 'NET LOSS' }}</td>
            <td class="num {{ $data['net_profit'] >= 0 ? 'net-positive' : 'net-negative' }}">Rs. {{ number_format(abs($data['net_profit']), 2) }}</td>
        </tr>
    </table>
