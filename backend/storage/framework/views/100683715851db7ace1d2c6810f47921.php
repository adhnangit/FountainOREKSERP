<?php $__env->startSection('title', 'Supplier Detail'); ?>
<?php $__env->startSection('page-title', 'Supplier Detail'); ?>
<?php $__env->startSection('page-desc', 'Supplier information and purchase history'); ?>
...
<div x-data="supplierShowPage()" x-init="init()">
    ...
    <div x-show="!loading" x-cloak>
        <a href="<?php echo e(url('/suppliers')); ?>" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 mb-4 transition-colors">
        ...
        <div class="cd-hero">
            <div class="flex items-start justify-between gap-4 relative" style="z-index:1">
                <div class="flex items-start gap-4 min-w-0">
                    <div class="cd-avatar" x-text="(supplier.name ?? '?').trim().charAt(0).toUpperCase()"></div>
                    <div class="min-w-0">
                        <div class="flex items-center gap-2.5 flex-wrap">
                            <h1 class="text-2xl font-bold truncate" x-text="supplier.name ?? ''"></h1>
                            <span class="cd-chip" :style="supplier.is_active === false ? 'background:rgba(239,68,68,.22);border-color:rgba(239,68,68,.3)' : 'background:rgba(34,197,94,.2);border-color:rgba(34,197,94,.3)'">
                                <span class="w-1.5 h-1.5 rounded-full" :style="supplier.is_active === false ? 'background:#fca5a5' : 'background:#86efac'"></span>
                                <span x-text="supplier.is_active === false ? 'Inactive' : 'Active'"></span>
                            </span>
                        </div>
                        <div class="flex items-center gap-2 mt-2.5 flex-wrap">
                            <span class="cd-chip" x-show="supplier.code"><span x-text="supplier.code"></span></span>
                            <span class="cd-chip" x-show="supplier.company" x-text="supplier.company"></span>
                            <span class="cd-chip" x-show="supplier.phone">
                            ...
                            <span class="cd-chip" x-show="supplier.city">
                                ...
                                <span x-text="supplier.city"></span>
                            </span>
                        </div>
                    </div>
                </div>
                <div class="flex items-center gap-2 flex-shrink-0">
                    <a :href="BASE + '/suppliers/' + id + '/bulk-payment'" class="cd-hdr-btn">
            ...
            <div class="cd-stat-row relative" style="z-index:1">
                ...
                <div class="cd-stat-card">
                    <div class="cd-stat-lbl">Payment Terms</div>
                    <div class="cd-stat-val text-gray-800 dark:text-gray-100" x-text="supplier.payment_terms_days ? supplier.payment_terms_days + ' days' : '—'"></div>
                </div>
                <div class="cd-stat-card" x-show="(supplier.credit_balance ?? 0) > 0">
                    <div class="cd-stat-lbl">Credit on Account</div>
                    <div class="cd-stat-val" style="color:#2563eb" x-text="fmtMoney(supplier.credit_balance ?? 0)"></div>
                </div>
            </div>
        </div>
        ...
        <div class="cd-tabs">
            ...
            <button @click="activeTab = 'orders'" class="cd-tab" :class="activeTab === 'orders' ? 'active' : ''">Purchase Orders</button>
        </div>
            ...
            <div class="card p-6">
                ...
                <div class="divide-y divide-gray-50 dark:divide-gray-800">
                    <div class="cd-info-row">
                        ...
                        <div><div class="cd-info-lbl">Phone</div><div class="cd-info-val" x-text="supplier.phone ?? '—'"></div></div>
                    </div>
                    <div class="cd-info-row">
                        ...
                        <div><div class="cd-info-lbl">Email</div><div class="cd-info-val" x-text="supplier.email ?? '—'"></div></div>
                    </div>
                    <div class="cd-info-row">
                        ...
                        <div><div class="cd-info-lbl">Address</div><div class="cd-info-val" x-text="[supplier.address, supplier.city, supplier.country].filter(Boolean).join(', ') || '—'"></div></div>
                    </div>
                    <div class="cd-info-row">
                        <div class="cd-info-icon"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></div>
                        <div><div class="cd-info-lbl">Contact Person</div><div class="cd-info-val" x-text="supplier.contact_person ?? '—'"></div></div>
                    </div>
                    <div class="cd-info-row">
                        ...
                        <div><div class="cd-info-lbl">Tax / VAT #</div><div class="cd-info-val" x-text="supplier.tax_number ?? '—'"></div></div>
                    </div>
                </div>
            </div>
        ...
        <div x-show="activeTab === 'orders'">
            <div class="card p-0 overflow-visible rounded-2xl">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
                    <h3 class="text-xs font-semibold uppercase text-gray-400 tracking-wider">Purchase Orders</h3>
                    <a :href="BASE + '/purchase-orders/create?supplier_id=' + id" class="btn-primary text-xs px-3 py-1.5">New PO</a>
                </div>
                <div x-show="loadingPOs" class="flex items-center justify-center py-12">
                ...
                <div x-show="!loadingPOs" class="overflow-x-auto">
                    <table class="cd-table min-w-full">
                        <thead>
                            <tr>
                                <th class="rounded-tl-2xl">PO #</th>
                                <th>Date</th>
                                <th>Expected</th>
                        ...
                        <tbody>
                            <template x-for="po in purchaseOrders" :key="po.id">
                                <tr>
                                    <td class="font-semibold text-indigo-600" x-text="po.po_number ?? ('#PO-' + po.id)"></td>
                                    <td x-text="fmtDate(po.order_date)"></td>
                                    <td x-text="fmtDate(po.expected_date)"></td>
                                    <td class="text-right font-semibold tabular-nums" x-text="fmtMoney(po.total_amount ?? 0)"></td>
                                    <td><span :class="poBadge(po.status)" x-text="po.status"></span></td>
                                    <td><a :href="BASE + '/purchase-orders/' + po.id" class="text-indigo-600 hover:underline text-sm font-medium">View</a></td>
                                </tr>
                            </template>
                            <tr x-show="purchaseOrders.length === 0 && !loadingPOs">
                                <td colspan="6" class="text-center text-gray-400 py-10">No purchase orders found.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div x-show="showEdit" class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10 overflow-y-auto" style="max-height:90vh">
            <div class="flex items-center justify-between mb-5">
                <h5 class="text-lg font-bold text-gray-800 dark:text-gray-100">Edit Supplier</h5>
            ...
            <div class="space-y-4">
                <div>
                    <label class="label text-xs">Name <span class="text-red-500">*</span></label>
                    <input type="text" x-model="editForm.name" class="input" />
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="label text-xs">Company</label>
                        <input type="text" x-model="editForm.company" class="input" />
                    </div>
                    <div>
                        <label class="label text-xs">Contact Person</label>
                        <input type="text" x-model="editForm.contact_person" class="input" />
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="label text-xs">City</label>
                        <input type="text" x-model="editForm.city" class="input" />
                    </div>
                    <div>
                        <label class="label text-xs">Payment Terms (days)</label>
                        <input type="number" x-model.number="editForm.payment_terms_days" min="0" class="input" />
                    </div>
                </div>
                <div>
                    <label class="label text-xs">Tax / VAT #</label>
                    <input type="text" x-model="editForm.tax_number" class="input" />
                </div>
                <div class="flex items-center gap-2">
                    <input type="checkbox" x-model="editForm.is_active" id="supp-active" class="rounded text-indigo-600" />
                    <label for="supp-active" class="text-sm text-gray-700 dark:text-gray-300">Active supplier</label>
                </div>
    ...
    <div x-show="showPayModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10 max-h-[90vh] overflow-y-auto">
            <div class="flex items-start justify-between mb-4">
                <div>
                    <h5 class="text-lg font-bold text-gray-800 dark:text-gray-100">Pay Opening Balance</h5>
                    <p class="text-sm text-gray-400 mt-0.5">Against outstanding balance — <span x-text="fmtMoney(supplier.balance ?? 0)"></span> owed</p>
                </div>
            ...
            <div class="space-y-4">
                ...
                <div x-show="payForm.payment_method === 'cheque'"
                    ...
                    <div class="text-xs font-semibold text-purple-600 uppercase tracking-wider mb-1">Cheque Details (issued by us)</div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
...
function supplierShowPage() {
    return {
        ...
        async init() {
            try {
                const sr = await apiFetch('/suppliers/' + this.id).then(r => r.json());
                this.supplier = sr.data ?? sr;
                this.purchaseOrders = this.supplier.purchase_orders ?? [];
            } catch (e) {
        ...
        async loadObPayments() {
            ...
            try {
                const data = await apiFetch('/suppliers/' + this.id + '/opening-balance-payments').then(r => r.json());
        ...
        async deleteObPayment(p) {
            if (!confirm(`Delete this ${fmtMoney(p.amount)} opening-balance payment${p.cheque ? ' (cheque ' + p.cheque.cheque_number + ')' : ''}? This restores the amount to what's owed.`)) return;
            try {
                const r = await apiFetch('/suppliers/' + this.id + '/opening-balance-payments/' + p.journal_entry_id, { method: 'DELETE' });
                const d = await r.json();
                if (!r.ok) { toast(d.message ?? 'Failed to delete payment.', 'error'); return; }
                toast('Payment deleted.', 'success');
                await this.loadObPayments();
                const fresh = await apiFetch('/suppliers/' + this.id).then(r => r.json());
                this.supplier = fresh.data ?? fresh;
            } catch (e) {
        ...
        openEdit() {
            ...
            this.editForm = {
                ...
                is_active: this.supplier.is_active !== false,
            };
            this.showEdit = true;
        },
        ...
        async saveEdit() {
            ...
            try {
                const r = await apiFetch('/suppliers/' + this.id, { method: 'PUT', body: JSON.stringify(this.editForm) });
                if (!r.ok) {
        ...
        async submitPayment() {
            ...
            if (this.payForm.amount > (this.supplier.balance ?? 0) + 0.01) {
                const proceed = confirm(`This payment (${fmtMoney(this.payForm.amount)}) is more than the ${fmtMoney(this.supplier.balance ?? 0)} currently owed. Continue anyway? (Check the payment history below first if you're not sure this hasn't already been recorded.)`);
                if (!proceed) return;
            }
            this.paySaving = true;
            try {
                const r = await apiFetch('/suppliers/' + this.id + '/opening-balance-payment', { method: 'POST', body: JSON.stringify(this.payForm) });
                const d = await r.json();
                if (!r.ok) { this.payError = d.message ?? Object.values(d.errors ?? {})[0]?.[0] ?? 'Failed to record payment.'; return; }
                this.supplier = { ...this.supplier, ...(d.supplier ?? {}), balance: d.outstanding_balance ?? this.supplier.balance };
        ...
        async deleteSupplier() {
            ...
            try {
                await apiFetch('/suppliers/' + this.id, { method: 'DELETE' });
        ...
        poBadge(status) {
            const map = { draft: 'badge-gray', confirmed: 'badge-primary', received: 'badge-success', partial: 'badge-warning', cancelled: 'badge-danger' };
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\xampp8.2\htdocs\FountainOREKS\backend\resources\views\suppliers\show.blade.php ENDPATH**/ ?>